...
/* ══════════════════════════════════════════════════════════════
   Cửa sổ giao diện — bấm cột trái để đổi vùng làm việc.
   Tiêu đề, phụ đề và nội dung từng màn lấy theo app thật
   (renderer/src/navigation.js + các panel component).

   Khung cửa sổ cố định chiều cao: đổi màn thì chỉ ruột đổi,
   không có gì nhảy — đúng cái mà section này đang nói.
   ══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  var main = document.getElementById('uiMain');
  var nav = document.querySelector('.ui-nav');
  if (!main || !nav) return;

  var tabs = [].slice.call(nav.querySelectorAll('button[data-v]'));
  if (!tabs.length) return;

  var reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  /* ── Nội dung 16 màn hình ─────────────────────────────────────
     Toàn bộ chuỗi dưới đây do mình viết sẵn, không có input người
     dùng, nên dựng bằng innerHTML là an toàn.                    */
  var V = {
    all: {
      t: 'Tất cả Profile', s: '312 profile · mỗi con một proxy và một bộ vân tay riêng',
      b: [['gh', 'Import token'], ['gh', 'Thêm profile'], ['pr', 'Mở đã chọn']],
      body: { k: 'rows', c: ['Tên', 'Nền tảng', 'Proxy', 'Trạng thái'], w: [27, 15, 32, 26], r: [
      ...
      st: [['Đã chọn', '6 profile'], ['Proxy sống', '298/312'], ['Token hết hạn', '4']]
    },

    discord: {
      t: 'Discord Hub', s: 'Lọc riêng account Discord · xem sức khoẻ từng con',
      b: [['gh', 'Kiểm tra token'], ['gh', 'Gộp thao tác'], ['pr', 'Đăng nhập lại']],
      body: { k: 'rows', c: ['Tên', 'Token', 'Tuổi acc', 'Sức khoẻ'], w: [27, 20, 18, 35], r: [
      ...
      st: [['Cờ tài khoản', 'không có con nào'], ['Acc quá non', '2 con bị giữ lại'], ['Nhảy vùng IP', '1 con · VN → TH']]
    },

    script: {
      t: 'Soạn thảo kịch bản', s: 'Thứ tự & nội dung tin nhắn cho từng tài khoản',
      b: [['gh', 'AI viết hộ'], ['gh', 'Mẫu'], ['pr', 'Chạy']],
      body: { k: 'script', r: [
      ...
      st: [['Nhịp gõ', 'tự nhiên · 38–72 WPM'], ['Chống trùng', 'đang bật'], ['Proxy', 'riêng từng acc']]
    },

    conversation: {
      t: 'Auto Chat (Auto Seeding)', s: 'Kịch bản tương tác đa tài khoản qua lại',
      b: [['gh', 'Chọn account'], ['gh', 'Số vòng: 4'], ['pr', 'Chạy vòng lặp']],
      body: { k: 'script', r: [
      ...
      st: [['Vòng lặp', '4 vòng'], ['Cách quãng', '3–11 phút'], ['Đảo vai', 'ngẫu nhiên mỗi vòng']]
    },

    tools: {
      t: 'Bộ công cụ Discord', s: '9 công cụ tự động hoá & tương tác',
      b: [['gh', 'Chọn account'], ['pr', 'Chạy công cụ']],
      body: { k: 'grid', g: [
      ...
      st: [['Chạy song song', '20 luồng'], ['Hạn mức', 'áp cho mọi công cụ'], ['Nhật ký', 'ghi lại từng lệnh']]
    },

    token: {
      t: 'Trích xuất Discord Token', s: 'Tự động đăng nhập & lấy Auth Token',
      b: [['gh', 'Dán danh sách'], ['pr', 'Bắt đầu lấy']],
      body: { k: 'rows', c: ['Email', 'Proxy đi kèm', 'Kết quả'], w: [36, 32, 32], r: [
      ...
      st: [['Đã lấy', '3/9'], ['Lưu vào profile', 'tự động'], ['Proxy', 'đi theo từng acc']]
    },

    inbox: {
      t: 'Hộp thư Discord', s: 'Đọc tin của tất cả account · real-time · tự động trả lời',
      b: [['gh', 'Bộ lọc'], ['gh', 'Auto-reply: bật'], ['pr', 'Gửi']],
      body: { k: 'inbox',
        th: [
      ...
      st: [['Chưa đọc', '9 tin'], ['Trả tự động', '6 tin hôm nay'], ['Đang trực', '312 account']]
    },

    listener: {
      t: 'Discord Gateway Listener', s: 'Nhận kịch bản gửi từ Claude qua Discord',
      b: [['gh', 'Kênh #traiga-hub'], ['pr', 'Ngắt kết nối']],
      body: { k: 'stream', r: [
      ...
      st: [['Kết nối', '4h12m không rớt'], ['Kịch bản nhận', '3 hôm nay'], ['Auto-Run', 'đang bật']]
    },

    browser: {
      t: 'Multi-Browser Matrix', s: 'Mở nhiều phiên trình duyệt với proxy & fingerprint độc lập',
      b: [['gh', 'Nạp proxy đã lưu'], ['gh', 'Soi anti-detect'], ['pr', 'Mở 8 phiên']],
      body: { k: 'grid', g: [
      ...
      st: [['Đang mở', '6/8 phiên'], ['Cookie', 'nạp sẵn, đăng nhập một lần'], ['Vân tay', 'khoá theo profile']]
    },

    ptest: {
      t: 'Kiểm tra Proxy', s: 'Đo latency & kiểm tra IP / vị trí địa lý',
      b: [['gh', 'Dán danh sách'], ['pr', 'Kiểm tra 312 proxy']],
      body: { k: 'rows', c: ['Proxy', 'IP ra', 'Vị trí', 'Độ trễ'], w: [28, 26, 24, 22], r: [
      ...
      st: [['Sống', '298 proxy'], ['Chết', '14 proxy'], ['Trễ trung bình', '118ms']]
    },

    pmon: {
      t: 'Giám sát Proxy', s: 'Theo dõi realtime — cảnh báo khi proxy chết',
      b: [['gh', 'Tự kiểm tra: 5 phút'], ['pr', 'Kiểm tra ngay']],
      body: { k: 'rows', c: ['Proxy', 'Uptime 24 giờ', 'Số lần rớt', 'Trạng thái'], w: [24, 34, 18, 24], r: [
      ...
      st: [['Tự kiểm tra', 'mỗi 5 phút'], ['Cảnh báo', 'đẩy về webhook'], ['Cần thay', '3 proxy']]
    },

    enrich: {
      t: 'Làm giàu hồ sơ', s: 'Avatar · display name · bio — account trắng dễ bị soi',
      b: [['gh', 'Chọn thư mục ảnh'], ['gh', 'Mẫu chữ'], ['pr', 'Chạy hàng loạt']],
      body: { k: 'meters',
        ti: [['Đã đổi avatar', '128', 'ok'], ['Đã đổi bio', '96', 'ok'], ['Còn trong hàng đợi', '88', 'wr']],
        ba: [
      ...
      st: [['Hạn mức', '12 acc mỗi giờ'], ['Ảnh', 'lấy ngẫu nhiên, không lặp'], ['Bio', 'sinh từ mẫu chữ của anh']]
    },

    warmup: {
      t: 'Warm-up account', s: 'Đọc · ack · react · đổi trạng thái · DM — rải theo thời gian',
      b: [['gh', 'Chọn account'], ['gh', 'Số vòng: 6'], ['pr', 'Bắt đầu warm-up']],
      body: { k: 'meters',
        ti: [['Đang nuôi', '46', 'ok'], ['Đủ tuổi ra trận', '271', 'ok'], ['Quá non, bị giữ', '2', 'wr']],
        lb: 'Độ ấm',
        ba: [
      ...
      st: [['Tiến độ', 'vòng 4/6'], ['Cách quãng', '8–22 phút mỗi acc'], ['Chặn', 'acc dưới 2 ngày tuổi']]
    },

    analytics: {
      t: 'Phân tích & Sức khoẻ trại', s: 'Hoạt động · tỉ lệ chết · proxy tương quan · nhảy vùng',
      b: [['gh', '14 ngày'], ['pr', 'Làm mới']],
      body: { k: 'meters',
        ti: [['Hoạt động 7 ngày', '1.284', 'ok'], ['Log 14 ngày', '8.902', 'in'], ['Proxy có acc chết', '3', 'wr']],
        ...
        ch: { lb: 'Hoạt động theo ngày', d: [42, 58, 51, 73, 66, 80, 47, 62, 88, 71, 94, 60, 77, 85], f: [2, 4, 1, 6, 3, 2, 9, 3, 2, 5, 1, 4, 2, 3] } },
      st: [['Acc chết dồn ở', 'bkk-77.res'], ['Nhảy quốc gia', '1 acc · VN → TH'], ['Ngân sách hôm nay', 'còn 41%']]
    },

    scheduler: {
      t: 'Hẹn giờ tác vụ', s: 'Lập lịch tự động chạy các tác vụ Discord',
      b: [['gh', 'Nhân bản'], ['pr', 'Thêm lịch']],
      body: { k: 'rows', c: ['Tác vụ', 'Khung giờ', 'Lệch ngẫu nhiên', 'Trạng thái'], w: [34, 24, 20, 22], r: [
      ...
      st: [['Đang bật', '7 lịch'], ['Lần chạy kế', '20:07 · đã lệch'], ['Vì sao lệch giờ', 'để nhịp không đều tăm tắp']]
    },

    log: {
      t: 'Lịch sử hoạt động', s: 'Theo dõi mọi hành động của profiles',
      b: [['gh', 'Lọc theo acc'], ['gh', 'Xuất CSV'], ['pr', 'Làm mới']],
      body: { k: 'stream', r: [
      ...
      st: [['Đã ghi', '8.902 dòng · 14 ngày'], ['Kèm theo', 'proxy dùng lúc đó'], ['Xuất', 'CSV cho từng acc']]
    }
  };
  ...
  function cell(c) {
    ...
    if (c[0] === 'm') return '<span class="c-m">' + c[1] + '</span>';
    if (c[0] === 'd') return '<span class="c-d cd-' + c[1] + '"><i class="dot d-' + c[1] + '"></i>' + c[2] + '</span>';
    if (c[0] === 'b') return '<span class="c-b"><i><u style="width:' + c[1] + '%"></u></i>' + c[2] + '</span>';
    return '';
  }

  var BODY = {
    rows: function (b) {
      var h = '<table class="ui-tb"><thead><tr>';
      for (var i = 0; i < b.c.length; i++) h += '<th style="width:' + b.w[i] + '%">' + b.c[i] + '</th>';
      h += '</tr></thead><tbody>';
      for (var r = 0; r < b.r.length; r++) {
        h += '<tr>';
        for (var j = 0; j < b.r[r].length; j++) h += '<td>' + cell(b.r[r][j]) + '</td>';
        h += '</tr>';
      }
    ...
    script: function (b) {
      var h = '<div class="ui-script">';
      for (var i = 0; i < b.r.length; i++) {
        var l = b.r[i];
        h += '<div class="ln' + (l[4] ? ' dim' : '') + '">' +
          '<span class="who ' + l[1] + '">' + l[0] + '</span>' +
          '<span class="msg">' + l[2] + '</span>' +
          '<span class="tm">' + l[3] + '</span></div>';
      }
      return h + '</div>';
    },

    grid: function (b) {
      var h = '<div class="ui-grid">';
      for (var i = 0; i < b.g.length; i++) {
        h += '<div class="gt"><i class="sq t-' + b.g[i][2] + '"></i>' +
          '<b>' + b.g[i][0] + '</b><span>' + b.g[i][1] + '</span>' +
          (b.g[i][3] ? '<em>' + b.g[i][3] + '</em>' : '') + '</div>';
      }
      return h + '</div>';
    },

    stream: function (b) {
      var h = '<div class="ui-stream">';
      for (var i = 0; i < b.r.length; i++) {
        h += '<div class="sl"><span class="st-t">' + b.r[i][0] + '</span>' +
          '<span class="st-k k-' + b.r[i][1] + '"></span>' +
          '<span class="st-m">' + b.r[i][2] + '</span></div>';
      }
      return h + '</div>';
    },

    inbox: function (b) {
      var h = '<div class="ui-inbox"><div class="ib-list">';
      for (var i = 0; i < b.th.length; i++) {
        var t = b.th[i];
        h += '<div class="ib-th' + (t[5] ? ' on' : '') + '">' +
          '<span class="ib-w">' + t[0] + (t[4] ? '<u>' + t[4] + '</u>' : '') + '</span>' +
          '<span class="ib-a">→ ' + t[1] + '</span>' +
          '<span class="ib-s">' + t[2] + '</span>' +
          '<span class="ib-t">' + t[3] + '</span></div>';
      }
      h += '</div><div class="ib-conv">';
      for (var j = 0; j < b.ms.length; j++) {
        var m = b.ms[j];
        h += '<div class="ib-m' + (m[0] ? ' out' : '') + '">' +
          '<span class="ib-mh">' + m[1] + ' · ' + m[3] + (m[4] ? ' · <em>' + m[4] + '</em>' : '') + '</span>' +
          '<span class="ib-mb">' + m[2] + '</span></div>';
      }
      return h + '</div></div>';
    },

    meters: function (b) {
      var h = '<div class="ui-met">';
      if (b.ti) {
        h += '<div class="mt-tiles">';
        for (var i = 0; i < b.ti.length; i++) {
          h += '<div class="mt-t"><b class="v-' + b.ti[i][2] + '">' + b.ti[i][1] + '</b><span>' + b.ti[i][0] + '</span></div>';
        }
        h += '</div>';
      }
      if (b.ba) {
        h += '<div class="mt-bars">';
        if (b.lb) h += '<p class="mt-lb">' + b.lb + '</p>';
        for (var j = 0; j < b.ba.length; j++) {
          h += '<div class="mt-b"><span>' + b.ba[j][0] + '</span>' +
            '<i><u style="width:' + b.ba[j][1] + '%"></u></i>' +
            '<em>' + b.ba[j][2] + '</em></div>';
        }
        h += '</div>';
      }
      if (b.ch) {
        h += '<div class="mt-ch"><p class="mt-lb">' + b.ch.lb + '</p><div class="ch-p">';
        for (var k = 0; k < b.ch.d.length; k++) {
          h += '<span class="ch-c"><b style="height:' + b.ch.f[k] + '%"></b>' +
            '<u style="height:' + b.ch.d[k] + '%"></u></span>';
        }
        h += '</div><p class="ch-lg"><span class="cl-a"></span>thành công<span class="cl-b"></span>lỗi</p></div>';
      }
      return h + '</div>';
    }
  };
  ...
  function paint(key) {
    ...
    var btns = '';
    for (var i = 0; i < v.b.length; i++) btns += '<span class="' + v.b[i][0] + '">' + v.b[i][1] + '</span>';

    var stat = '';
    for (var j = 0; j < v.st.length; j++) stat += '<div><b>' + v.st[j][0] + '</b><span>' + v.st[j][1] + '</span></div>';

    main.innerHTML =
      '<div class="ui-head"><div class="ui-ttl"><h3>' + v.t + '</h3><p>' + v.s + '</p></div>' +
      '<div class="ui-btns">' + btns + '</div></div>' +
      '<div class="ui-fill">' + BODY[v.body.k](v.body) + '</div>' +
      '<div class="ui-stat">' + stat + '</div>';

    if (!reduced) {
      main.classList.remove('swap');
      void main.offsetWidth;   // ép trình duyệt chạy lại animation
      main.classList.add('swap');
    }
  }

  /* ── Tab: một cái mở, còn lại đóng ──────────────────────────── */
  function select(tab, focus) {
    ...
    paint(tab.getAttribute('data-v'));
    if (focus) tab.focus();
  }

  for (var i = 0; i < tabs.length; i++) {
    tabs[i].id = 'uitab-' + tabs[i].getAttribute('data-v');
    tabs[i].setAttribute('role', 'tab');
    tabs[i].setAttribute('aria-selected', 'false');
    tabs[i].tabIndex = -1;
    tabs[i].addEventListener('click', function () { select(this, false); });
  }
  ...
  nav.addEventListener('keydown', function (e) {
    ...
    if (at < 0) return;
    var to = -1;
    if (e.key === 'ArrowDown' || e.key === 'ArrowRight') to = (at + 1) % tabs.length;
    else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') to = (at - 1 + tabs.length) % tabs.length;
  ...
  select(nav.querySelector('.on') || tabs[0], false);

  /* ── Script đang chạy vẫn nhích, dù anh đang xem màn nào ─────
     Đây chính là câu trong phần dẫn: tiến độ nằm ngay dưới thanh
     điều hướng, không phụ thuộc vùng làm việc.                   */
  var runs = [].slice.call(document.querySelectorAll('.ui-runs .run'));
  if (runs.length && !reduced) {
    var NAMES = ['gaconlonton', 'mikevu_92', 'bap_ngo', 'linhpham_vn', 'tho_con_04', 'haiyen_2k'];
    var jobs = runs.map(function (el, n) {
      return { el: el, name: el.querySelector('span'), bar: el.querySelector('.bar div'),
               num: el.querySelector('u'), cur: n ? 7 : 16, tot: 25 };
    });
    var step = function () {
      for (var k = 0; k < jobs.length; k++) {
        ...
        if (j.cur > j.tot) {
          j.cur = 1;
          j.tot = 18 + Math.floor(Math.random() * 14);
          j.name.textContent = NAMES[Math.floor(Math.random() * NAMES.length)];
        }
        j.bar.style.width = Math.round((j.cur / j.tot) * 100) + '%';
        j.num.textContent = j.cur + '/' + j.tot;
      }
    };
    var runTimer = setInterval(step, 1700);

    document.addEventListener('visibilitychange', function () {
      if (document.hidden) { clearInterval(runTimer); runTimer = null; }
      else if (!runTimer) { runTimer = setInterval(step, 1700); }
    });
  }
})();
